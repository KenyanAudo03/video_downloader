const splitLayout = document.getElementById('splitLayout');
  ...
  const currentVideoDownloadBtn = document.getElementById('currentVideoDownloadBtn');
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    currentVideos.forEach((video, index) => {
      ...
      if (videoId) {
        ...
        const title = video.querySelector('.searched-video-title')?.textContent || 'Unknown Title';
  ...
  function highlightCurrentVideo(videoId) {
    ...
    const currentVideoCards = document.querySelectorAll(`[data-video-id="${videoId}"]`);
  ...
  function startVideoEndDetection() {
    ...
    videoEndCheckInterval = setInterval(() => {
      if (sideVideoFrame.contentWindow) {
        try {
          sideVideoFrame.contentWindow.postMessage('{"event":"listening","id":"player"}', '*');
        } catch (error) {
  ...
  function handleIframeLoad() {
    // Start the restricted video skip timer when iframe loads
    // This will be cleared if the video starts playing normally
    startRestrictedVideoSkipTimer();
  }

  function findCurrentVideoIndex() {
    return allLoadedVideos.findIndex(video => video.id === currentPlayingVideoId);
  }
  ...
  function openVideoPlayer(videoId, title) {
    ...
    if (currentVideoInfo) {
      ...
      const existingKeyboardInfo = currentVideoInfo.querySelector('.keyboard-controls-info');
      ...
      const keyboardInfoHTML = `
        <div class="keyboard-controls-info" style="margin-top: 10px; padding: 8px; background: rgba(255,255,255,0.1); border-radius: 4px; font-size: 12px; color: #ccc;">
          ...
          <span style="display: inline-block; margin-right: 15px;">⏪⏩ <strong>:</strong> Seek backward/forward in video</span><br>
    ...
    currentVideos.forEach(videoCard => {
      ...
      const channelName = videoCard.querySelector('.searched-video-meta span').textContent;
      const publishedTime = videoCard.querySelector('.searched-video-meta span:last-child').textContent;
      ...
      const sideVideoHTML = `
        <div class="searched-video-card" data-video-id="${videoId}" onclick="switchVideo('${videoId}', '${title.replace(/'/g, "\\'")}')">
          <div class="searched-video-thumbnail">
            ${thumbnailImg ? `<img class="searched-thumbnail-img" src="${thumbnailImg.src}" alt="${title}" loading="lazy" />` : '<div class="no-thumbnail">No thumbnail</div>'}
            ${duration ? `<div class="searched-video-duration">${duration.textContent}</div>` : ''}
          </div>
  ...
  function downloadVideo(videoId, title) {
    ...
    const downloadUrl = `/search/download-video/?id=${videoId}&title=${encodeURIComponent(title)}`;
  ...
  function createSideVideoCard(video) {
    ...
    return `
      <div class="searched-video-card" data-video-id="${video.id}" onclick="switchVideo('${video.id}', '${video.title.replace(/'/g, "\\'")}')">
        <div class="searched-video-thumbnail">
          ${thumbnailUrl ? 
  ...
  function createVideoCard(video) {
    ...
    return `
      <div class="searched-video-card" data-video-id="${video.id}">
        <div class="searched-video-thumbnail" onclick="openVideoPlayer('${video.id}', '${video.title.replace(/'/g, "\\'")}')">
          ${thumbnailUrl ? 
            `<img class="searched-thumbnail-img" src="${thumbnailUrl}" alt="${video.title}" loading="lazy" />` : 
            '<div class="no-thumbnail">No thumbnail</div>'
          }
